...
#endif
			if(doRefine) {
				this->refineDetails(pMembership, pIdxMap, pSeg, planeCountous);
#ifdef EVAL_SPEED
			...
			while(!minQ.empty() && step<=maxStep) {
				...
				for(; itr!=p->nbs.end();itr++) {//test merge with all nbs, pick the one with min mse
					...
					if(cand_merge==0 || cand_merge->mse>merge->mse ||
						(cand_merge->mse==merge->mse && cand_merge->mse <merge->mse))
					{
						cand_merge=merge;
						cand_nb=nb;
					}
